...
def compute_and_add_ber(pil_rgb_image, method_string, method_dict_to_save_to, scales, enc_length, watermark_actual_bits):
...
with open("results.txt", "w") as f:
    ...
    for img_path in tqdm(img_paths):
        for method_string, SCALE in METHODS_AVAILIABLE:
            ...
            tmp_savepath = f"../laion1024_results/{method}/watermarked_{slug}".replace(".jpg", ".png")
            ...
            starttime = time.time()
            decoder = WatermarkDecoder('bits', enc_length)
            recovered_watermark = [int(bit) for bit in decoder.decode(to_decode_bgr, method_string, scales=scales)]
            ...
            if TEST_ATTACKS:
                ...
                # jpg attack
                tmp_savepath_jpg = f"../laion1024_results/{method}/watermarked_{slug}.jpg"
                watermarked_rgb_copy.save(tmp_savepath_jpg)
                jpg_img_rgb = Image.open(tmp_savepath)
                compute_and_add_ber(jpg_img_rgb, method_string, attack_jpg_ber, scales, enc_length, watermark_bits)
                os.remove(tmp_savepath_jpg)

                # horizontal flip attack
                horizontal_flip = watermarked_rgb.copy().transpose(Image.FLIP_LEFT_RIGHT)
                compute_and_add_ber(horizontal_flip, method_string, attack_horizonal_flip_ber, scales, enc_length, watermark_bits)

                # vertical flip attack
                vertical_flip = watermarked_rgb.copy().transpose(Image.FLIP_TOP_BOTTOM)
                compute_and_add_ber(vertical_flip, method_string, attack_vertical_flip_ber, scales, enc_length, watermark_bits)
                ...
                cropped_image = watermarked_rgb.copy().crop((start_x, start_y, start_x + crop_size[0], start_y + crop_size[1]))
                compute_and_add_ber(cropped_image, method_string, attack_random_crop_ber, scales, enc_length, watermark_bits)

                # noise attack
                image_array = np.array(watermarked_rgb.copy())
                noise_scale = 16 
                noise = np.random.randint(-noise_scale, noise_scale, image_array.shape, dtype='int32')
                noisy_image_array =  np.clip(image_array + noise, 0, 255)
                noisy_image = Image.fromarray(noisy_image_array.astype('uint8')) 
                compute_and_add_ber(noisy_image, method_string, attack_random_noise_ber, scales, enc_length, watermark_bits)

                # rotate attack
                rotated_image = watermarked_rgb.copy().rotate(-90)
                compute_and_add_ber(rotated_image, method_string, attack_rotate_ber, scales, enc_length, watermark_bits)
    ...
    for method_string, SCALE in METHODS_AVAILIABLE:
        ...
        f.write(f"\tFlip (horizontal): {np.mean(attack_horizonal_flip_ber[method]):.2f}\n")
    ...
    for i in range(len(encode_ms)):
        plt.text(encode_ms[i] + offset * 0.4, psnrs[i] + offset * 0.4, methods[i], fontsize=9, ha='left', va='top')
    ...
    # Adding labels to each point
    for i in range(len(encode_ms)):
        plt.text(encode_ms[i] + offset, perfect_decodes[i] + offset, methods[i], fontsize=9, ha='left', va='top')
    ...
    # Adding labels to each point
    for i in range(len(encode_ms)):
        plt.text(encode_ms[i] + offset, passing[i] + offset, methods[i], fontsize=9, ha='left', va='top')
    ...
    for i in range(len(psnrs)):
        plt.text(psnrs[i] + offset, passing[i] + offset, methods[i], fontsize=9, ha='left', va='top')
    ...
    # Adding labels to each point
    for i in range(len(psnrs)):
        plt.text(psnrs[i] + offset, mean_ber[i] + offset, methods[i], fontsize=9, ha='left', va='top')
